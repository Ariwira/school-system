...
export async function generateMetadata({ params }: SchoolStudentDetailPageProps) {
  const { subAppKey, id } = await params
  return {
    title: `Detail Siswa — ${id} — ${subAppKey} — School ERP`,
  }
}
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="outline" size="sm" render={<Link href={`/school/${subAppKey}/students`} />}>
      ...
      <Card>
        ...
        <CardContent>
          <StudentPaymentHistory studentId={student.id} subAppKey={subAppKey} />
        </CardContent>
      </Card>
    </div>
  )
